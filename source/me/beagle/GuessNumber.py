...
while True: # 这个就是强制循环来模拟do while,条件写在循环体内
    ...
    guessNumInput = input("猜猜数组内的数字:") # 输入文字
    while not guessNumInput.isdigit(): # 判断输入的内容是不是数字类型
        print(guessNumInput,"不是数字") # 上述判断如果不是数字类型则打印该字符并告知不是数字
        guessNumInput = input("猜猜数组内的数字:") # 同时给用户提供新的输入框等待输入

    guessNum = int(guessNumInput) # 若是数字类型则转换为整数型并压入对象内

    # if not guessNum in numList: # 下列判断的第二种写法
    if guessNum not in numList: # 判断该对象是否不属于列表内的值
        print(guessNumInput + " 不是列表里的数字哦") # 若不属于则打印该数字并提示
        countor += 1 # 同时计数器自增1
        continue # 并继续从循环头开始执行
    else: # 若该对象属于列表内的值
        print("猜对了:", guessNumInput) # 则打印猜对了的值
        break # 并且终止循环
